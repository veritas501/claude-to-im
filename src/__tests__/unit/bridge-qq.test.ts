...
describe('bridge-manager - computeSdkSessionUpdate', () => {
  ...
  it('keeps sdkSessionId on error to preserve conversation context', async () => {
    const { computeSdkSessionUpdate } = await import('../../lib/bridge/bridge-manager');
    const result = computeSdkSessionUpdate('new-sdk-123', true);
    assert.equal(result, 'new-sdk-123', 'Error with SDK ID: should keep it to preserve context');
  });

  it('returns null on error without sdkSessionId (keep existing binding)', async () => {
    const { computeSdkSessionUpdate } = await import('../../lib/bridge/bridge-manager');
    const result = computeSdkSessionUpdate(null, true);
    assert.equal(result, null, 'Error without SDK ID: keep existing binding value');
  });
