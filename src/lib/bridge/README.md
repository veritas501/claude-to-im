...
A host-agnostic bridge that connects IM platforms (Telegram, Discord, Feishu/Lark, QQ) to Claude, enabling AI-powered conversations through messaging apps.

## Features

- **Multi-platform**: Telegram (long polling), Discord (Gateway), Feishu/Lark (WSClient), QQ (WebSocket)
...
See `host.ts` for full interface definitions and `examples/mock-host.ts` for a reference implementation.
