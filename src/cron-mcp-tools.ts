/**
 * Cron MCP tool definitions for in-process SDK MCP server.
 *
 * Provides a `cron` tool with actions: add, list, remove, update, status.
 * Used by the LLM provider to inject cron management tools into each Claude session.
 *
 * Bridge context (channelType, chatId, userId, displayName) is passed per-call
 * via closure, not via environment variables — enabling correct channel routing
 * even when multiple sessions run concurrently.
 *
 * Storage: ~/.claude-to-im/data/cron.json (shared with the daemon cron-scheduler).
 */
...
// ── Public: build SDK MCP server ──

/**
 * Build an in-process SDK MCP server with the `cron` tool.
 * Bridge context is captured via closure so each query session
 * gets its own channel routing.
 */
export function buildCronMcpServer(bridgeContext?: BridgeContext) {
  const ctx: BridgeCtx = bridgeContext
    ? {
  ...
  return createSdkMcpServer({
    ...
    tools: [
      {
        name: 'cron',
        description: `Manage scheduled/cron jobs for the claude-to-im bridge. Jobs execute prompts through Claude and deliver results to the IM channel where the job was created.
