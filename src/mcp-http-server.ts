/**
 * MCP HTTP/SSE Server for cron job management.
 *
 * Runs inside the daemon process and exposes the `cron` tool via SSE transport.
 * Claude CLI connects via SSE, bridge context passed per-session through
...
import type { BridgeContext } from './lib/bridge/host.js';
...
type BridgeCtx = { channelType: string; chatId: string; userId?: string; displayName?: string } | null;
...
function handleAdd(params: Record<string, unknown>, ctx: BridgeCtx): string {
...
function handleList(params: Record<string, unknown>, ctx: BridgeCtx): string {
  const store = loadStore();
...
// ── MCP Server factory (per-connection bridge context) ──

function createCronMcpServer(ctx: BridgeCtx): McpServer {
    ...
    async (params: Record<string, unknown>) => {
      ...
      try {
        switch (params.action) {
          case 'add':
            result = handleAdd(params, ctx);
            break;
          case 'list':
            result = handleList(params, ctx);
            break;
          case 'remove':
...
// Active SSE sessions — needed to route POST messages back to the correct transport
const sessions = new Map<string, { transport: SSEServerTransport; server: McpServer }>();

/**
 * Start the MCP HTTP/SSE server on the given port (0 = random free port).
 * Returns the actual port.
 */
export function startMcpHttpServer(port: number = 0): Promise<number> {
  return new Promise((resolve, reject) => {
    if (httpServer) {
      reject(new Error('MCP HTTP server already running'));
      return;
    }
    ...
    httpServer = http.createServer(async (req, res) => {
      ...
      // ── OAuth discovery — tell Claude CLI no auth is required ──
      if (req.method === 'GET' && (
        ...
        // Return empty/invalid metadata so the client skips OAuth
      ...
      if (req.method === 'GET' && url.pathname === '/sse') {
        // Extract bridge context from query parameter
        let ctx: BridgeCtx = null;
        const ctxParam = url.searchParams.get('ctx');
        if (ctxParam) {
          try {
            ctx = JSON.parse(decodeURIComponent(ctxParam));
          } catch { /* ignore */ }
        }

        const sessionId = crypto.randomUUID();
        const transport = new SSEServerTransport('/message?session=' + sessionId, res);
        const mcp = createCronMcpServer(ctx);

        sessions.set(sessionId, { transport, server: mcp });
        console.log(`[mcp-http] SSE connected (session=${sessionId}, ctx=${ctx ? ctx.channelType + ':' + ctx.chatId : 'none'})`);

        res.on('close', () => {
          sessions.delete(sessionId);
          console.log(`[mcp-http] SSE disconnected (session=${sessionId})`);
        });
    ...
    httpServer.listen(port, '127.0.0.1', () => {
      ...
      console.log(`[mcp-http] MCP SSE server listening on http://127.0.0.1:${actualPort}`);
...
export function stopMcpHttpServer(): Promise<void> {
...
/** Get SSE URL for the running server (null if not running). */
export function getMcpSseUrl(): string | null {
  if (!httpServer) return null;
  const addr = httpServer.address();
  if (!addr || typeof addr !== 'object') return null;
  return `http://127.0.0.1:${addr.port}/sse`;
}

/** Build SSE URL with bridge context query param. */
export function buildMcpSseUrl(bridgeContext?: BridgeContext): string | null {
  const baseUrl = getMcpSseUrl();
  if (!baseUrl) return null;
  if (!bridgeContext) return baseUrl;
  const ctx = encodeURIComponent(JSON.stringify({
    channelType: bridgeContext.channelType,
    chatId: bridgeContext.chatId,
    userId: bridgeContext.userId,
    displayName: bridgeContext.displayName,
  }));
  return `${baseUrl}?ctx=${ctx}`;
}
